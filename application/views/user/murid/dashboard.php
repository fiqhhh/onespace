...
<header x-data="{open: false}" class="bg-white shadow">
    ...
    <nav class="container flex justify-between px-8 py-5 mx-auto">
        ...
        <div class="cursor-pointer">
            <div class="-mt-2">
                <a href="<?= base_url('murid/profile'); ?>"><img class="w-10 p-1 border rounded-full border-secondary-300" src="<?= base_url('assets/img/foto_pengguna/') . $murid['foto_pengguna']; ?>" alt="avatar"></a>
            </div>
        </div>
    </nav>
...
<main class="container px-8 pt-8 mx-auto mt-8">
    ...
    <div class="mt-0">
        <form action="<?= base_url('murid/gabung_kelas'); ?>" method="POST" autocomplete="off">
            <input class="w-full px-4 py-3 bg-transparent border-2 border-b-4 focus:outline-none rounded-primary border-secondary-300" name="token_kelas" type="text" placeholder="Masukan kode untuk masuk kelas . . .">
        </form>
    </div>
    ...
    <div class="mt-10">
        ...
        <?php
        $absensi_masuk = $this->m_absen->TampilAbsenMasuk($murid['id_murid'])->result();
        ?>

        <?php if (empty($absensi_masuk)) { ?>
            <a href="<?= base_url('murid/absen_murid_masuk'); ?>" class="flex justify-center p-5 mt-4 mb-6 text-white border-2 border-b-8 focus:bg-tertiary-200 bg-tertiary-300 rounded-primary border-secondary-300">
        ...
        <?php }else{ ?>
            <?php foreach ($absensi_masuk as $row) { ?>
                ...
                <?php if (empty($row->tgl_absen_keluar) && empty($row->wkt_absen_keluar)) { ?>
                    ...
                    <?php if ($row->id_murid != $murid['id_murid']) { ?>
                        <a href="<?= base_url('murid/absen_murid_masuk'); ?>" class="flex justify-center p-5 mt-4 mb-6 text-white border-2 border-b-8 focus:bg-primary-200 bg-tertiary-300 rounded-primary border-secondary-300">
                            <div class="text-center">
                                <div class="text-xl font-semibold text-secondary-300">Absensi Masuk</div>

                                <div class="flex justify-center p-1 border-2 mb-1 rounded-primary border-secondary-300 bg-danger-300">
                                    <svg class="w-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"></path>
                                    </svg>
                                </div>
                    ...
                    <?php } else { ?>
                        <?php if(date('H:i:s') >= "17:00:00" ){ ?>
                            <a href="<?= base_url('murid/absen_keluar_masuk/') . $row->id_absen_masuk; ?>" class="flex justify-center p-5 mt-4 mb-6 text-white border-2 border-b-8 focus:bg-primary-200 bg-tertiary-300 rounded-primary border-secondary-300">
                                <div class="text-center">
    ...
    <section class="mt-6">
        <?php if(empty($kelas)){ ?>
            <a href="#" class="flex justify-center mt-5 p-5 mb-5 border-2 border-b-8 focus:bg-primary-200 bg-primary-300 rounded-primary border-secondary-300 text-white">
                <div>
                    <h3 class="text-3xl font-semibold">Belum bergabung dengan kelas</h3>
                </div>
            </a>
        <?php }else{ ?>
            <?php foreach ($kelas as $key) { ?>
                ...
                <div class="flex justify-between p-5 mb-5 text-white border-2 border-b-8 bg-primary-300 rounded-primary border-secondary-300">
                    <a href="<?= base_url('murid/kelas/') . $key->id_kelas; ?>" class="hover:underline focus:underline">
                    ...
                    <?php $absen_hari_ini = $this->m_absen->MapelHariIni($murid['id_murid'], $key->id_kelas)->result(); ?>
